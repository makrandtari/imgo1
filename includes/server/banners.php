...
foreach ($images as $file) {
    ...
    if($img_height == '3'){ $imgReW = 1400; $imgReH = 575; }
  ...
  switch ($uploadImageType) {
            ...
            case IMAGETYPE_PNG:
                ...
                png2jpg($fileName, $uploadPath, $fileNameRKN);
                break;
            ...
            case IMAGETYPE_WEBP:
                ...
                createOptImgThumnail($sourceProperties, $resourceType, $sourceImageWidth, $sourceImageHeight, $uploadPath, $uploadPathWebP, $fileNameRKN, $fileExt, $optimizeImageThumbQualityJPG, $optimizeImageThumbQualityWebp, $optimizeImageThumbSize, $imgTrans, 'imagejpeg');
                webp2jpg($fileName, $uploadPath, $fileNameRKN);
                break;
...
function png2jpg($filePath, $uploadPath, $fileName){
  ...
  imagedestroy($image);
  $quality = 95;
  imagejpeg($bg, $uploadPath.$fileName.".jpg", $quality);
  imagedestroy($bg);
}

function webp2jpg($filePath, $uploadPath, $fileName){
  $im = imagecreatefromwebp($filePath);
  $quality = 95;
  imagejpeg($im, $uploadPath.$fileName.".jpg", $quality);
  imagedestroy($im);
}